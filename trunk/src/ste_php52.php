...
class ste_cache_null implements ste_cache
{
...
class ste_cache_file extends ste_cache_null implements ste_cache
{
	public $path;
	public $memcache = array();
...
class node
{
	...
	static public function sgenerate(&$that) {
		...
		if (isset($that->ref)) return self::sgenerate($that->ref);

		if (!$that->is_root) $that->ste->process_block($that, 'RENDER');
		$s = '';
		$s .= self::sgenerate($that->a);
		foreach ($that->b as $e) $s .= self::sgenerate($e);
		$s .= self::sgenerate($that->c);
		return $s;
	}
	...
	public function createnode() {
		return new self($this->node_parser);
	}
